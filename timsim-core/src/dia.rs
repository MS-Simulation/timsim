...
use crate::containers::{IsotopeTransmissionConfig, IsotopeTransmissionMode};
use crate::projector::{IntensityStage, MzCoordSpace, RenderedEvent, RenderedSpectrum};
use crate::scheme::{DataMode, InstrumentCapabilities, IsolationWindow};
use crate::handle::{FragmentIonsWithComplementary, TimsTofSyntheticsDataHandle};
use crate::precursor::TimsTofSyntheticsPrecursorFrameBuilder;
...
impl TimsTofSyntheticsFrameBuilderDIA {
    ...
    pub fn new_with_config(
        ...
        Self::new_with_config_and_source(
            ...
            num_threads,
            isotope_config,
            &crate::projector::DistributionSource::Columns,
        )
    }
    ...
    pub fn new_with_config_and_source(
        ...
        num_threads: usize,
        isotope_config: IsotopeTransmissionConfig,
        source: &crate::projector::DistributionSource,
    ) -> rusqlite::Result<Self> {
        ...
        for (peptide_id, frame_abundance) in peptide_ids.iter().zip(frame_abundances.iter()) {
            ...
            for (index, ion_abundance) in ion_abundances.iter().enumerate() {
                ...
                for (scan, scan_abundance) in all_scan_occurrence.iter().zip(all_scan_abundance.iter()) {
                    ...
                    let Some(collision_energy_quantized) = crate::handle::resolve_fragment_ce_key(
                        ...
                        if crate::handle::fragment_prefix_exists(fragment_ions, *peptide_id, charge_state) {
                            panic!(
    ...
    pub fn render_fragment_scan(
        ...
        for (peptide_id, frame_abundance) in peptide_ids.iter().zip(frame_abundances.iter()) {
            ...
            for (index, ion_abundance) in ion_abundances.iter().enumerate() {
                ...
                let Some(ce_key) = crate::handle::resolve_fragment_ce_key(
                    fragment_ions,
                    *peptide_id,
                    charge_state,
                    nce,
