...
| [`timsim-schema`](timsim-schema) | The **Arrow/Parquet column contract**: 17 tables across four axes (structure / quantity / design / measurement), a `SCHEMA_VERSION`, and — the point of the crate — **validation on read** (`read`, `read_stream`, `validate`) so a stage rejects a wrong input *before* it computes, with a column-level message. Column names are constants (`peptides::MASS_MONOISOTOPIC`), never string literals in stage code. Also the **integrity gate** (`integrity`, `verify`, `timsim-verify`): every artifact's *logical values* are hashed on the way into the writer and recorded in a `<file>.integrity.json` sidecar, so a shifted row is caught — which Parquet page CRCs and ZSTD frame checksums structurally cannot do, both being computed after the bad values reach the writer. |
